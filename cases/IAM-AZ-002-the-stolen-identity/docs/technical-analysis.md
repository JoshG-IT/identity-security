...
The Entra ID and OAuth mechanics behind the investigation. Environment-specific values are replaced with placeholders. Microsoft's publicly documented identifiers are also represented as placeholders, so that resolving them remains work the reader does.
...
Microsoft's publicly documented identifiers, including the Microsoft Graph application ID and the Graph permission GUIDs, are also represented as placeholders. They are not sensitive and are identical in every tenant. They are substituted because resolving an identifier to a permission name is the technique this file explains, and a reader who looks the values up has performed that technique rather than read its answer.
